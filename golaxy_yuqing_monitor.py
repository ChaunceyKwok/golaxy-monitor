...
# 注: 财付通小贷标签常被判成微信支付/分付/财付通, 单靠业务标签不可靠, 故用"财付通小贷"原词在 标题+正文 匹配。
CFT_XIAODAI_KW = ["财付通小贷", "财付通网络金融小额贷款", "财付通小额贷款"]
...
GUARD_IF_PRESENT = {
    ...
    # 社保/新开户: 拦社保代缴/银行拉新软文, 误伤"微信提现手续费补贴"报道(列举银行卡时提"含社保卡"、绑卡"新开户")
    # 同篇含微信提现/零钱提现等核心业务词时豁免。(修复2026-07-10: 金融时报《银行报销微信提现手续费》被误杀)
    "社保": ["微信提现", "提现手续费", "微信零钱", "零钱提现", "提现至", "微信支付", "零钱通", "理财通"],
    "新开户": ["微信提现", "提现手续费", "微信零钱", "零钱提现", "提现至", "微信支付", "零钱通", "理财通"],
    # 融资: 拦创投融资类无关财经(路演/天使轮/A轮/VC/IPO), 误伤腾讯系公司融资动态。
    # 同篇含腾讯系核心主体时豁免。(修复2026-07-17: 禾金财经《财付通小贷100亿ABN融资》被误杀)
    "融资": ["财付通", "财付通小贷", "微信支付", "腾讯理财通", "腾讯金融科技", "腾安基金", "微众银行", "微粒贷", "微民保险", "腾讯"],
    # ABS/REITs/可转债/公司债/企业债: 拦无关创投金融术语, 误伤腾讯系公司(财付通小贷等)真实融资工具动态。
    # 同篇含腾讯系核心主体时豁免。(修复2026-07-17: 财付通小贷ABS/ABN发行动态被"ABS"误杀)
    "ABS": ["财付通", "微信支付", "腾讯理财通", "腾讯金融科技", "腾安基金", "微众银行", "微粒贷", "腾讯"],
    "REITs": ["财付通", "微信支付", "腾讯理财通", "腾讯金融科技", "腾安基金", "微众银行", "微粒贷", "腾讯"],
    "可转债": ["财付通", "微信支付", "腾讯理财通", "腾讯金融科技", "腾安基金", "微众银行", "微粒贷", "腾讯"],
    "公司债": ["财付通", "微信支付", "腾讯理财通", "腾讯金融科技", "腾安基金", "微众银行", "微粒贷", "腾讯"],
    "企业债": ["财付通", "微信支付", "腾讯理财通", "腾讯金融科技", "腾安基金", "微众银行", "微粒贷", "腾讯"],
    # 上市: 拦IPO/科创板/纳斯达克等无关财经, 误伤"ABN产品上市流通"(资产支持票据在交易所上市, 非公司IPO)。
    # 同篇含腾讯系核心主体时豁免。(修复2026-07-21: 北京商报《财付通小贷百亿ABN》"26和盛8号ABN001上市流通"被误杀)
    "上市": ["财付通", "微信支付", "腾讯理财通", "腾讯金融科技", "腾安基金", "微众银行", "微粒贷", "腾讯"],
    # 体育/赛事词: 拦体育赛事无关内容, 误伤"用户拿赛事类比理财/押注"的UGC(如"押冠军押在腾讯理财通")。
    # 同篇含腾讯理财核心业务词时豁免。(修复2026-07-21) 注: "世界杯"已直接从排除词移除(用户指定), 不在此豁免。
    "奥运会": ["理财通", "零钱通", "微信理财", "腾讯理财通", "腾安基金", "微信支付", "微信零钱"],
    "亚运会": ["理财通", "零钱通", "微信理财", "腾讯理财通", "腾安基金", "微信支付", "微信零钱"],
    "欧冠": ["理财通", "零钱通", "微信理财", "腾讯理财通", "腾安基金", "微信支付", "微信零钱"],
    "总冠军": ["理财通", "零钱通", "微信理财", "腾讯理财通", "腾安基金", "微信支付", "微信零钱"],
    # 消费节: 拦电商促销大促软文(618/双11/消费节), 误伤"微信支付响应政策推商户扶持/入境支付便利"的官方业务报道。
    # 同篇含腾讯系核心主体时豁免。(修复2026-07-22: 金融时报《微信支付扫码盒子+入境支付便利两大专项》被"消费节"误杀)
    "消费节": ["微信支付", "财付通", "腾讯理财通", "腾讯金融科技", "腾安基金", "微信零钱", "零钱通", "腾讯"],
}
...
FORCE_PUSH_KW = ["stockbuddy"]

# 🏛️ 党央媒署名白名单(2026-07-22 用户指定强化开白):
# 命中党央媒署名词 + 命中腾讯品牌词(COMPOUND_CHECK) → 强开白: 跳过排除词/SPAM/UGC/subtag黑名单等误杀关,
#   仅保留开户引流软文(PROMO_KW)拦截, 防止营销号冒用党媒署名的广告混入。
# 说明: 党媒原稿常被自媒体转载(author/captureWebsite为转载号), 故署名锚点用 标题+正文 文本匹配。
AUTHORITY_MEDIA = [
    # 金融类党媒
    "金融时报", "中国金融新闻网", "中国银行保险报", "金融时报-",
    # 中央党政媒
    "人民日报", "人民网", "新华社", "新华网", "央视", "cctv", "中央广播电视总台",
    "光明日报", "经济日报", "求是", "央广网", "中新社", "中国新闻社", "中国新闻网",
    # 证券官媒
    "中国证券报", "上海证券报", "证券时报", "经济参考报",
    # 主流市场化财经
    "第一财经", "21世纪经济报道", "财新", "南方都市报", "南方周末",
]
...
# 权威资讯/舆情类 subtag: 命中品牌时可豁免单个 SPAM 词误杀(区别于软文类subtag)。
# 这些是监管/融资/信贷/证券等正经资讯或舆情, 非运营软文。2026-07-17 新增。
NEWS_SUBTAGS = ["监管政策与合规动态", "信贷与催收舆情", "支付及信贷", "证券与投资动态",
                "动态事件", "动态事件-监管政策与合规动态", "跨境支付与汇款",
                "数字人民币动态", "零钱通及基金异动"]
...
def push_one(it):
    ...
    if ok and NEGATIVE_ALERT and it.get("_senti") == -1:
        ...
        elif is_mobile_event or is_fenfu_biz or is_cft_xiaodai:
    ...
    # 移动话费充值事件(非负面: 中性/我方口径) → 单独 @ anderschen(陈恩达)
        ...
        try:
            ...
            log(f"   移动事件@提醒失败: {e}")
    # 财付通小贷(非负面: 正面/中性) → 单独 @ anderschen(陈恩达)
    elif ok and is_cft_xiaodai and MOBILE_EVENT_AT_USERIDS:
        at_text = (f"💳 财付通小贷相关舆情｜{it.get('matched_kw','-')}\n"
                   f"{it.get('title','')[:40]}\n@anderschen 请关注。")
        payload = {"msgtype": "text", "text": {"content": at_text,
                                               "mentioned_list": list(MOBILE_EVENT_AT_USERIDS)}}
        try:
...
def run_once(asts, excludes, block_media, block_url):
    ...
    for src in docs:
        ...
        # ⚡⚡ 强制放行通道 (最高优先级): 命中 FORCE_PUSH_KW(如 stockbuddy) 即推送,
        # 跳过所有过滤关卡(排除词/UGC/SPAM/subtag黑名单/URL黑名单等)。大小写不敏感。
        # (2026-07-14 用户指定: stockbuddy 一旦出现就要推送)
        _text_fp = src.get("text") or ""
        _combined_fp = f"{title_raw} {_text_fp}".lower()
        if any(k.lower() in _combined_fp for k in FORCE_PUSH_KW):
        ...
        # 🏛️ 党央媒署名 强开白通道 (2026-07-22 用户指定): 命中党媒署名 + 腾讯品牌词 → 强制推送,
        # 跳过排除词/SPAM/UGC/subtag黑名单等误杀关; 仅保留开户引流软文(PROMO_KW)拦截, 防营销号冒名广告。
        _am_blob = f"{title_raw} {src.get('text') or ''}"
        _hb_am = [c for c in COMPOUND_CHECK if c.lower() in _am_blob.lower()]
        if _hb_am and any(a.lower() in _am_blob.lower() for a in AUTHORITY_MEDIA):
        ...
        if not is_negative:
            ...
            if is_spam:
                ...
                # 防误杀3: 权威资讯/舆情类 subtag(监管政策/信贷催收/证券投资等) + 命中品牌 → 正经资讯, 放行。
                #   区别于软文类subtag(产品软文与开通引流/产品知识及活动)。
                #   (修复2026-07-17: 财付通小贷100亿ABN融资资讯 因"资金周转"命中SPAM被误杀)
                is_news_subtag = _subtag in NEWS_SUBTAGS
                _hit_brand_spam = any(c.lower() in combined for c in COMPOUND_CHECK)
                if (not is_complaint
                        and not (has_licai_core and has_licai_signal)
                        and not (is_news_subtag and _hit_brand_spam)):
                    excluded += 1; continue
