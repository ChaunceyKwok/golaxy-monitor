...
# Golaxy API  (GitHub Actions 版: 敏感信息从环境变量/Secrets 读取, 不硬编码进仓库)
...
MONITOR_TEXT = "腾讯金融科技 微信支付 财付通 腾讯理财通 零钱通 微信零钱通 腾安基金 腾讯自选股 微信分付 微企付 微信零钱 微信提现"

# ES match 分词后 ("微信支付"→"微信"+"支付") 太宽泛，本地加复合词校验
COMPOUND_CHECK = ["腾讯金融科技", "微信支付", "财付通", "腾讯理财通", "理财通", "微信理财", "零钱通", "微信零钱通",
                  "腾安基金", "腾讯自选股", "微信分付", "微企付",
                  "微信零钱", "微信提现"]
...
def make_summary(title, text, hit_words, max_len=160):
    ...
    if not sentences:
        return text[:max_len]
    hit_set = [w for w in dict.fromkeys(hit_words) if w]
    STRONG = ("微信支付", "微信", "财付通", "腾讯", "理财通", "零钱通", "微众",
              "微保", "微粒贷", "wepay", "tenpay", "分付")
    strong_sents, weak_sents = [], []
    for s in sentences:
        sl = s.lower()
        if not any(w.lower() in sl for w in hit_set):
            continue
        if any(b.lower() in sl for b in STRONG):
            strong_sents.append(s)
        else:
            weak_sents.append(s)
    ordered = []
    seen_s = set()
    for s in strong_sents + weak_sents:
        if s not in seen_s:
            ordered.append(s); seen_s.add(s)
    if sentences[0] not in seen_s:
...
def build_item_v2(src):
    ...
    # 摘要: 优先 API summary, 回退规则式
    summary = (src.get("summary") or "").strip()
    if not summary or len(summary) < 20:
        summary = make_summary(title, text, [], 160)
...
def run_once(asts, excludes, block_media, block_url):
    ...
    for src in docs:
        ...
        # 覆盖 matched_kw: 用真正命中的业务名(零钱通/微信支付等), 不是 API 的分类 tag
        item["matched_kw"] = " / ".join(hit_brands) if hit_brands else item.get("matched_kw","舆情")
        matched.append(item)
        batch_fps.add(fp)
